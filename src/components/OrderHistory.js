...
const OrderItem = ({ order }) => {
    ...
    return (
        ...
        <tr style={{fontWeight:400}}>
            ...
            <td style={{ fontWeight: 500 , textAlign: 'center', color: order.orderStatus === 'cancelled' ? 'red' : order.orderStatus === 'delivered' ? 'green' : '#e6b802 ' }}>
                {order.orderStatus}
            </td>
...
const OrderHistory = () => {
    ...
    const fetchOrders = () => {
        axios
            .get(`${base_URL}/order/customerId/1`)
            .then((response) => {
                if (response.data) {
